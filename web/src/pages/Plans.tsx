...
export function Plans() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setMessage('')
      try {
        const [config, info, response, rewardConfig, rewardHistory] = await Promise.all([
  ...
  }, [authData, baseUrl, setAdmobConfig, setPlans, setProfile, setRewardLogs])
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-5 p-6 pb-24">
      ...
      {(planRewardAdEnabled || !paymentEnabled) && (
        <section className="space-y-3 rounded-2xl bg-surface-low p-5 shadow-sm border border-outline-variant/40">
          ...
          {adRewardLogs.filter((log) => log.scene === 'plan').length > 0 && (
            <ul className="space-y-2 border-t border-outline-variant/20 pt-3">
              {adRewardLogs.filter((log) => log.scene === 'plan').slice(0, 3).map((log) => (
