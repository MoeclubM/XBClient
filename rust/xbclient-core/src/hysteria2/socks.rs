...
use super::protocol::{Hysteria2Stream, SocksTarget};
...
pub(crate) async fn start_socks(
    config: Hysteria2Config,
    listen: String,
) -> Result<(u64, SocketAddr)> {
    let listener = TcpListener::bind(&listen)
        .await
        .with_context(|| format!("bind Hysteria2 SOCKS listener on {listen}"))?;
    let local_addr = listener
        .local_addr()
        .context("read Hysteria2 SOCKS listener address")?;
    let session_id = NEXT_SESSION_ID.fetch_add(1, Ordering::SeqCst);
    let clients = Arc::new(StdMutex::new(Vec::new()));
    let (shutdown_tx, shutdown_rx) = oneshot::channel();
    SOCKS_SESSIONS
        .lock()
        .expect("Hysteria2 SOCKS session map lock poisoned")
        .insert(
            session_id,
            SocksSession {
...
pub(crate) async fn stop_socks(session_id: u64) -> Result<String> {
    let session = SOCKS_SESSIONS
        ...
        .expect("Hysteria2 SOCKS session map lock poisoned")
        .remove(&session_id)
        .with_context(|| format!("Hysteria2 SOCKS session not found: {session_id}"))?;
    let _ = session.shutdown.send(());
    for client in session
        .clients
        .lock()
        .expect("Hysteria2 SOCKS client list lock poisoned")
        .drain(..)
    {
        client.abort();
    }
                    ...
                    Ok((stream, _)) => {
                        ...
                        let client = tokio::spawn(async move {
                            if let Err(error) = handle_socks_client(stream, config).await {
                                log::error!("Hysteria2 SOCKS client error: {error:?}");
                            }
                        });
                        let mut clients = clients.lock().expect("Hysteria2 SOCKS client list lock poisoned");
                        clients.retain(|client| !client.is_finished());
                        clients.push(client);
                    }
                    Err(error) => {
                        log::error!("Hysteria2 SOCKS listener error: {error:?}");
                        break;
                    },
                }
            }
...
async fn handle_socks_client(mut stream: TcpStream, config: Hysteria2Config) -> Result<()> {
    match read_socks_request(&mut stream).await? {
        SocksRequest::Connect(target) => {
            let remote = match Hysteria2Stream::connect(config, target).await {
                Ok(remote) => remote,
                Err(error) => {
...
async fn read_socks_request(stream: &mut TcpStream) -> Result<SocksRequest> {
    ...
    stream
        ...
        .await
        .context("read Hysteria2 SOCKS greeting")?;
    ensure!(header[0] == 0x05, "only SOCKS5 is supported");
    let mut methods = vec![0u8; header[1] as usize];
    stream
        .read_exact(&mut methods)
        .await
        .context("read Hysteria2 SOCKS methods")?;
    stream
        .write_all(&[0x05, 0x00])
        .await
        .context("write Hysteria2 SOCKS method response")?;

    let mut request = [0u8; 4];
    stream
        .read_exact(&mut request)
        .await
        .context("read Hysteria2 SOCKS request")?;
...
async fn relay(stream: TcpStream, remote: Hysteria2Stream) -> Result<()> {
    let (mut remote_send, mut remote_recv, _client) = remote.into_parts();
    ...
    let uplink = async {
        ...
        loop {
            let read = local_reader
                .read(&mut buffer)
                .await
                .context("read local Hysteria2 SOCKS payload")?;
            if read == 0 {
    ...
    let downlink = write_remote_payloads(&mut remote_recv, local_writer);
