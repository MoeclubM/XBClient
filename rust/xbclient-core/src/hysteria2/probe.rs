...
mod platform {
    ...
    pub async fn test(input: &str) -> Result<String> {
        let request: TestNodeRequest =
            serde_json::from_str(input).context("parse Hysteria2 node test request")?;
    ...
    async fn test_connection(
        ...
        .await
        .context("Hysteria2 target test timed out")?
    }
    ...
    async fn flush_tls(tls: &mut ClientConnection, stream: &mut Hysteria2Stream) -> Result<()> {
    ...
    async fn read_tls(tls: &mut ClientConnection, stream: &mut Hysteria2Stream) -> Result<()> {
        let packet = stream
            .read_payload()
            .await?
...
mod platform {
    ...
    pub async fn test(input: &str) -> Result<String> {
        let _request: TestNodeRequest =
            serde_json::from_str(input).context("parse Hysteria2 node test request")?;
        anyhow::bail!("Hysteria2 node test is only available on Android target builds")
    }
}
