...
mod platform {
    ...
    pub async fn start(input: &str) -> Result<String> {
        let request: StartVpnRequest =
            serde_json::from_str(input).context("parse start Hysteria2 VPN request")?;
        ...
        VPN_SESSIONS
            .lock()
            .expect("Hysteria2 VPN session map lock poisoned")
            .insert(
                session_id,
                VpnSession {
                    socks_session_id,
        ...
        tokio::spawn(async move {
            ...
            if let Err(error) = result {
                log::error!("Hysteria2 tun2proxy exited with error: {error:?}");
            }
    ...
    pub async fn stop(session_id: u64) -> Result<String> {
        let session = VPN_SESSIONS
            ...
            .expect("Hysteria2 VPN session map lock poisoned")
            .remove(&session_id)
            .with_context(|| format!("Hysteria2 VPN session not found: {session_id}"))?;
...
mod platform {
    ...
    pub async fn start(input: &str) -> Result<String> {
        let _request: StartVpnRequest =
            serde_json::from_str(input).context("parse start Hysteria2 VPN request")?;
        bail!("Hysteria2 Android VPN is only available on Android target builds")
    }
